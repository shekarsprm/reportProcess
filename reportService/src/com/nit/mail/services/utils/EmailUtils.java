package com.nit.mail.services.utils;
...
import com.nit.mail.services.helpers.MailServicesConstants;
...
public static Logger LOGGER=Logger.getLogger(EmailUtils.class);	
	public static Integer emailUtils(String userEmailId){
		...
		try {
			...
			properties.put("mail.smtp.host", MailServicesConstants.SERVER_HOST);
			properties.put("mail.smtp.port",MailServicesConstants.SERVER_PORT);
			...
			Authenticator auth = new Authenticator() {
				public PasswordAuthentication getPasswordAuthentication() {
					try {
						return new PasswordAuthentication(MailServicesConstants.FROM_EMAIL_ID,AESEncrpUtils.decrypt(MailServicesConstants.FROM_PWD));
					} catch (Exception e) {
						e.printStackTrace();
						return null;
					}
			...
			msg.setFrom(new InternetAddress(MailServicesConstants.FROM_EMAIL_ID));
